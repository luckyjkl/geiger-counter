...
void SPI_Flash_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(SPI_FLASH_CS_RCC, ENABLE);

	GPIO_InitStructure.GPIO_Pin = SPI_FLASH_CS;  //SPI CS
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(SPI_FLASH_CS_GPIO, &GPIO_InitStructure);
 	GPIO_SetBits(SPI_FLASH_CS_GPIO, SPI_FLASH_CS);
	
	RCC_APB2PeriphClockCmd(Flash_GPIO_RCC, ENABLE);		//GPIO,SPI使能
	RCC_APB1PeriphClockCmd(Flash_SPI_RCC, ENABLE);

	GPIO_InitStructure.GPIO_Pin = Flash_SPI_SCK | Flash_SPI_MISO | Flash_SPI_MOSI;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;								//复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(Flash_SPI_GPIO, &GPIO_InitStructure);									//GPIO初始化
	GPIO_SetBits(Flash_SPI_GPIO, Flash_SPI_SCK | Flash_SPI_MISO | Flash_SPI_MOSI);	//IO输出高
	...
	SPI_InitType.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4;		//预分频4
	SPI_InitType.SPI_FirstBit = SPI_FirstBit_MSB;						//传输数据从MSE位开始
	SPI_InitType.SPI_CRCPolynomial = 7;									//CRC值计算的多项式
	SPI_Init(Flash_SPI, &SPI_InitType);										//初始化SPI1
...
{ 
	...
	if(NumByteToWrite<=secremain)secremain=NumByteToWrite;//不大于扇区剩余空间
	while(1) 
	{	
		...
		if(i<secremain)//需要擦除
		{
			...
			{
				SPI_FLASH_BUF[secoff+i]=pBuffer[i];	  
			}
...
{
	u8 retry;
	while(SPI_I2S_GetFlagStatus(Flash_SPI, SPI_I2S_FLAG_TXE) == RESET)		//检查指定SPI的发送缓存空标志位
	{
		retry++;
		if(retry >= 200)
			return 0;
	}
	SPI_I2S_SendData(Flash_SPI, TxData);			//通过外设发送一个数据
	retry = 0;
	
	while(SPI_I2S_GetFlagStatus(Flash_SPI, SPI_I2S_FLAG_RXNE) == RESET)		//检查指定SPI的接受缓存非空标志位
	{
		retry++;
		if(retry >= 200)
			return 0;
	}
	return SPI_I2S_ReceiveData(Flash_SPI);		//返回SPI接受到的数据		
}
